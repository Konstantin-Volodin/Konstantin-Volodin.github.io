...
function ProjectCard(props: any) {
  ...
  return (
    <VisibilitySensor onChange={onChange} partialVisibility={true}>
      <SlideFade
        in={enteredScreen}
        offsetY={prefersReducedMotion ? '0px' : '60px'}
          ...
          >
            <Box
              ...
              _hover={{
                ...
                outline: '1px solid var(--chakra-colors-brand-200)',
              }}
...
const GROUPS: Record<string, (item: any) => boolean> = {
  All: () => true,
  Web: i =>
    (i.skills || []).includes('Web Development') ||
    (i.technologies || []).some((t: string) => ['React', 'Django'].includes(t)),
  ML: i =>
    (i.skills || []).includes('Data Science') ||
    (i.technologies || []).some((t: string) =>
      ['PyTorch', 'Statsforecast'].includes(t)
    ),
  Optimization: i =>
    (i.skills || []).some((s: string) => s.toLowerCase().includes('optimiz')) ||
    (i.technologies || []).some((t: string) => ['Gurobi'].includes(t)),
  Analytics: i =>
    (i.skills || []).some((s: string) =>
      ['Data Analysis', 'Visualization', 'Dashboarding'].includes(s)
    ),
  Cloud: i =>
    (i.technologies || []).some((t: string) => ['GCP', 'AWS'].includes(t)) ||
    (i.skills || []).includes('Cloud Providers'),
};
...
function Projects() {
  ...
  const allTechs = useMemo(() => {
    ...
    projData.forEach(
      (i: any) =>
        Array.isArray(i.technologies) &&
        i.technologies.forEach((t: string) => set.add(t))
    );
    const TOP_TECHS = [
      'Python',
      'R',
  ...
  const [activeTech, setActiveTech] = useState(() => {
    ...
    if (
      ...
      (fromUrl === 'All' || allTechs.includes(fromUrl))
    )
      return fromUrl;
    try {
      ...
      if (
        typeof saved === 'string' &&
        (saved === 'All' || allTechs.includes(saved))
      )
        return saved;
    } catch {}
    return 'All';
  ...
  const syncSelection = useCallback(
    (cat: keyof typeof GROUPS, tech: string, replace = true) => {
      try {
  ...
  useEffect(() => {
    const onPop = () => {
      ...
      const nextTech =
        typeof techFromUrl === 'string' &&
        (techFromUrl === 'All' || allTechs.includes(techFromUrl))
          ? techFromUrl
          : 'All';
      setActiveTech(nextTech);
      try {
        localStorage.setItem(STORAGE_TECH_KEY, nextTech);
      } catch {}
    };
  ...
  const hasActiveFilters = useMemo(
    () => activeGroup !== 'All' || activeTech !== 'All',
  ...
  const items = useMemo(() => {
    ...
    if (activeTech && activeTech !== 'All') {
      arr = arr.filter(
        (i: any) =>
          Array.isArray(i.technologies) && i.technologies.includes(activeTech)
      );
    }
    return arr;
  }, [activeGroup, activeTech]);
  ...
  const slugs = useMemo(
    () => items.map((i: any) => slugify(i.name)),
    [items, slugify]
  );
